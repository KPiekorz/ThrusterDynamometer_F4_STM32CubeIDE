 ...
 * @param[in]			-
 *
 * @return				- initialization status (1 - OK)
 *
 * @Note				- none
 *
 *****************************************************************************************************/
...
void MS5837_ADC_result(I2C_HandleTypeDef *hi2c){
	...
	OFF = ((float)C[2] * 65536) + (((float)C[4] * dT) / 128);
	SENS = ((float)C[1] * 32768) + ((float)C[3] * dT / 256);
	P = ((((float)D1 * SENS)/2097152) - OFF) / 8192;
	...
	SENS2 = SENS - SENSi;
	TEMP = (TEMP - Ti);
	P = ((((float)D1 * SENS2)/2097152) - OFF2) / 8192;
